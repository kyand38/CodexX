...
router.get('/game/:search', async (_req: Request, res: Response) => {
  try {
    const response = await fetch(`${process.env.REQUEST_URL}/games?search=${_req.params.search}&key=${process.env.API_KEY}`);
    const data = await response.json();
    res.json(data);
  } catch (error: any) {
    res.status(500).json({ message: error.message });
